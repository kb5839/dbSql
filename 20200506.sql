...
SLECT 시도, 시군구, (KFC 스칼라 서브쿼리), (버거킹 스칼라 서브쿼리) .....
FROM ...
...
1.테이블의 모든 컬럼에 값을 입력하는 경우, 컬럼명을 나열하지 않아도 된다
    단 입력할 값으 순서는 테이블에 정의된 컬럼 순서로 입력된다.
...
3.SELECT 결과를 INSERT
  ...
  사용자로 부터 데이터를 직접 입력 받는 경우(ex회원가입)는 적용이 불가
...
테이터 입력을 취소 하려면 : rollback - 트랜잭션 취소
rollback;
SELECT*
FROM dept;
...
empno가 9999이고 ename은 본인이름, hiredate는 현재 일시를 저장하는 INSERT 구문 작성
...
INSERT - 없던 것을 새로 생성
UPDATE,DELETE - 기존에 있는 것을 변경,삭제
==>쿼리를 작성할 경우 주의사항
    1. WHERE절이 누락되지 않았는지
    2. UPDATE,DELETE 문을 실행하기전에 WHERE절을 복사해서 SELECT에서 실행하여 체크해본다
...
1.특정 컬럼만 값을 삭제 ==> 해당 컬럼을 NULL로 UPDATE
    DELETE절은 행 자체를 삭제
2.UPDATE와 마찬가지로 DELETE 쿼리를 실행하기 전에 SELECT를 통해 삭제 대상이 되는 행을 조회,확인
